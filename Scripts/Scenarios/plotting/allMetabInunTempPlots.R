...
# Animations --------------------------------------------------------------
  ...
  inunAnim <- ggplot() +
    ...
    # scale_fill_gradient(low = 'firebrick', high = 'forestgreen' ) +
    # scale_fill_viridis(option = 'mako') +
    scale_fill_continuous_sequential(palette = 'YLGnBu', end = 0.8) +
    # scale_fill_continuous_sequential(palette = 'Blue-Yellow') +
    # gganimate bits
    # only the times that exist, but less smooth
    # gganimate bits
    transition_states(time) +
    labs(title = 'Volume wetlands inundated {closest_state}', fill = 'log(Volume)') +
    # # Smooth time, but has lots of extra 'times' that aren't in the data
    # transition_time(time) +
  ...
  # https://colorspace.r-forge.r-project.org/articles/approximations.html#overview-1
    ...
    # gganimate bits
    transition_states(time) +
    labs(title = 'Mean wetland temperature {closest_state}', fill = 'Temp') +
    # # Smooth time, but has lots of extra 'times' that aren't in the data
    # transition_time(time) +
  ...
  gppValleyAnim <- ggplot() +
    ...
    # gganimate bits
    transition_states(time) +
    labs(title = 'Total wetland GPP {closest_state}', fill = 'GPP') +
    # # Smooth time, but has lots of extra 'times' that aren't in the data
    # transition_time(time) +
  ...
  erValleyAnim <- ggplot() +
    ...
    # gganimate bits
    transition_states(time) +
    labs(title = 'Total wetland ER {closest_state}', fill = 'log(ER)') +
    # # Smooth time, but has lots of extra 'times' that aren't in the data
    # transition_time(time) +
  ...
  autotrophAnim <- ggplot() +
    ...
    # gganimate bits
    transition_states(time) +
    labs(title = 'Autotrophic index {closest_state}', fill = 'AT') +
    # # Smooth time, but has lots of extra 'times' that aren't in the data
    # transition_time(time) +
    # labs(title = 'ER {as.Date(frame_time)}') + #,
    # fill = paste0('log(', statname, 'inundated)')) +
