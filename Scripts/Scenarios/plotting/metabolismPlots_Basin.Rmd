...
inun_gg <- inunfun(inundationannual/1000, 1, datewanted, units = 'Gl', plotPkg = 'ggplot', titled = FALSE, titlePrefix = 'Water year beginning ', 
                   forcelegend = 'GL Inundation summed\nover max extents', forcemin = 10000, forcemax = 100000000)
inun_gg
...
gpp_gg <- gppfun(logGPPdaysannual/1000, 1, datewanted, units = 'tonnes', plotPkg = 'ggplot', titled = FALSE, titlePrefix = 'Water year beginning ', 
                   forcelegend = 'GPP (tonnes O2)\nsummed over max extents', forcemin = 10000, forcemax = 100000000)
gpp_gg
```
...
eryrs <- erfun(logERdaysannual/1000, 1, datewanted = as.character(availDays[2:4]), 
               ...
               forcemin = 10000, forcemax = 100000000) + 
  guides(fill = guide_legend(title.position = 'top')) +
  theme_grey(base_size = 8) + 
...
```{r}
inun5label <- 'GL Inundation summed\nover max extents'
temp5label <- '2014-19 mean\ntemp C'
gpp5label <- 'GPP (tonnes O2)\nsummed over max extents'
er5label <- 'ER (tonnes O2)\nsummed over max extents'
```
...
gridIO_5 <- temp_5 +
  ...
  er_5 +
    ...
    theme(legend.position = 'right', 
          legend.background = element_blank(),
          legend.key.size = unit(0.3, 'cm')) + 
  plot_annotation(title = paste0('2014-2019 water years'))
...
iC <- inunsetup(inun5df, attnum = 'logInun', forcemin = 10000, forcemax = 100000000)
gC <- gppsetup(gpp5df, attnum = 'logGPP', forcemin = 10000, forcemax = 100000000)
eC <- ersetup(er5df, attnum = 'logER', forcemin = 10000, forcemax = 100000000)
```
...
#### Annual timeseries
```{r}
tempAseries <- ggplot(tempannualdf, aes(x = date, y = Temp, color = ValleyName)) + 
  geom_point() + geom_line() + 
  labs(x = NULL, y = 'Temp C') 
inunAseries <- ggplot(inunannualdf, aes(x = date, y = logInun, color = ValleyName)) + 
  geom_point() + geom_line() + scale_y_continuous(breaks = eiC,
                    # limits = c(min(eiC), max(eiC)),
                    labels = format(10^eiC, big.mark=",",
                                    scientific=FALSE, trim = TRUE, digits = 0))+
  labs(x = NULL, y = 'Inundation\n(summed GL at max)')
gppAseries <- ggplot(gppannualdf, aes(x = date, y = logGPP, color = ValleyName)) + 
  geom_point() + geom_line() + scale_y_continuous(breaks = eiC,
                    # limits = c(min(eiC), max(eiC)),
                    labels = format(10^eiC, big.mark=",",
                                    scientific=FALSE, trim = TRUE, digits = 0))+
  labs(x = NULL, y = 'GPP\n(summed tonnes O2/day at max)')
erAseries <- ggplot(erannualdf, aes(x = date, y = logER, color = ValleyName)) + 
  geom_point() + geom_line() + scale_y_continuous(breaks = eiC,
                    # limits = c(min(eiC), max(eiC)),
                    labels = format(10^eiC, big.mark=",",
                                    scientific=FALSE, trim = TRUE, digits = 0))+
...
```{r}
tempAseries + inunAseries + gppAseries + erAseries + plot_layout(guides = 'collect')
```
...
```{r}
ggpubr::ggarrange(gridIO_5,
                  gppAseries + theme(legend.position = 'none'), nrow = 2)
```
...
```{r}
basintime <- ggplot(filter(gluedataA, variable != 'Temp'), 
       aes(x = date,
           y = value, 
           color = variable)) + 
  geom_point() + geom_line() +
  scale_color_manual(values = c(eC$erpal[length(eC$erpal)-2], gC$gpppal[length(gC$gpppal)-2], iC$inunpal[8]),
                    ...
                    # guide = 'legend',
                    name = NULL) +
  # scale_y_continuous(breaks = eC$erbreaks,
  #                   # limits = c(min(eC$erbreaks), max(eC$erbreaks)),
  #                   labels = format(10^eC$erbreaks, big.mark=",",
  #                                   scientific=FALSE, trim = TRUE, digits = 0)) +
  # theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(x = NULL, y = NULL)
basintime
...
```{r}
eryrsAll <- erfun(logERdaysannual/1000, 1, datewanted = as.character(availDays), 
               units = 'tonnes', 
               plotPkg = 'ggplot', 
               titled = FALSE, 
               titlePrefix = 'Water year beginning ',
  ...
  theme(legend.position = 'top', 
       legend.background = element_blank(),
       legend.key.size = unit(0.3, 'cm'))+ 
  facet_grid(~date) # override the wrap()
...
```{r}
ermapandtime <- eryrsAll / erAseries + 
  xlim(c(as.Date('2015-03-01'), as.Date('2019-09-01'))) + # Cut to match the dates in the facets
  labs(y = 'ER') + # it's just too long
  theme(legend.position = 'none')
# ggpubr::ggarrange(gridIO_5,
#                   gppAseries + theme(legend.position = 'none'), nrow = 2)
ermapandtime
...
(tempBase_5 + ggtitle('Baseline temp') + theme(legend.position = 'none')) +
  (tempClim_5 + ggtitle('2c warming') + theme(legend.position = 'none')) +
  
  (inunBase_5 + ggtitle('Baseline inun') + theme(legend.position = 'none')) +
...
**WHY DOESN"T A 10% INCREASE IN INUN = 10% INCREASE IN GPP? IT'S JUST MULTIPLIED**
