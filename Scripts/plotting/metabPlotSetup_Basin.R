...
source(file.path(here::here(), 'directorySet.R'))
...
# check
all(
  st_get_dimension_values(inundation, which = 'time') ==
    st_get_dimension_values(temperature, which = 'time')
)

all(
  st_get_dimension_values(inundation10p, which = 'time') ==
    st_get_dimension_values(temperature, which = 'time')
)
...
for (i in 1:length(starobjs)) {
  assign(
    ...
    st_set_dimensions(
      get(starobjs[i]),
      which = 'time',
      values = as.Date(st_get_dimension_values(
        get(starobjs[i]),
        which = 'time'
      ))
    )
...
temperatureannual <- tempaggregate(
  temperature,
  by_t = as.Date(interDates),
  FUN = meanna,
  na.rm = TRUE
...
inundationannual <- tempaggregate(
  inundation,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
  ...
  aperm(c('Shape', 'time'))
inundation10pannual <- tempaggregate(
  inundation10p,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdays10pannual <- tempaggregate(
  logERdays10p,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdays10pCLIMannual <- tempaggregate(
  logERdays10pCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdaysCLIMannual <- tempaggregate(
  logERdaysCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleysannual <- tempaggregate(
  logERdaysvalleys,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleys10pannual <- tempaggregate(
  logERdaysvalleys10p,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleys10pCLIMannual <- tempaggregate(
  logERdaysvalleys10pCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
) %>%
...
logERdaysvalleysCLIMannual <- tempaggregate(
  logERdaysvalleysCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
) %>%
...
# max to get mostin the bimonth (sum isn't right for the inundation, since it's max extent)
logGPPdaysannual <- tempaggregate(
  logGPPdays,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logGPPdays10pannual <- tempaggregate(
  logGPPdays10p,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logGPPdays10pCLIMannual <- tempaggregate(
  logGPPdays10pCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysCLIMannual <- tempaggregate(
  logGPPdaysCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleysannual <- tempaggregate(
  logGPPdaysvalleys,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleys10pannual <- tempaggregate(
  logGPPdaysvalleys10p,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
) %>%
...
logGPPdaysvalleys10pCLIMannual <- tempaggregate(
  logGPPdaysvalleys10pCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
) %>%
...
logGPPdaysvalleysCLIMannual <- tempaggregate(
  logGPPdaysvalleysCLIM,
  by_t = as.Date(interDates),
  FUN = maxna,
  na.rm = TRUE
) %>%
...
rbind(
  st_get_dimension_values(inundation, which = 'time'),
  findInterval(td, by_t, rightmost.closed = TRUE)
)
...
logERdays10pCLIM5 <- tempaggregate(
  logERdays10pCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleys5 <- tempaggregate(
  logERdaysvalleys,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleys10p5 <- tempaggregate(
  logERdaysvalleys10p,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleys10pCLIM5 <- tempaggregate(
  logERdaysvalleys10pCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logERdaysvalleysCLIM5 <- tempaggregate(
  logERdaysvalleysCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logGPPdays10pCLIM5 <- tempaggregate(
  logGPPdays10pCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleys5 <- tempaggregate(
  logGPPdaysvalleys,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleys10p5 <- tempaggregate(
  logGPPdaysvalleys10p,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleys10pCLIM5 <- tempaggregate(
  logGPPdaysvalleys10pCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
logGPPdaysvalleysCLIM5 <- tempaggregate(
  logGPPdaysvalleysCLIM,
  by_t = by_t_5,
  FUN = maxna,
  na.rm = TRUE
...
temp5df <- sfandcatch(
  temperature5,
  polyinfo = ltimNoNorth,
  newname = 'Temp'
) %>%
  mutate(center = st_centroid(Shape), latpos = st_coordinates(center)[, 2])

clim5df <- sfandcatch(climate5, polyinfo = ltimNoNorth, newname = 'Temp') %>%
  mutate(center = st_centroid(Shape), latpos = st_coordinates(center)[, 2])
...
er510pCLIMdf <- sfandcatch(
  logERdays10pCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er5CLIMdf <- sfandcatch(
  logERdaysCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er5_v10pdf <- sfandcatch(
  logERdaysvalleys10p5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er5_v10pCLIMdf <- sfandcatch(
  logERdaysvalleys10pCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er5_vCLIMdf <- sfandcatch(
  logERdaysvalleysCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
gpp510pdf <- sfandcatch(
  logGPPdays10p5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp510pCLIMdf <- sfandcatch(
  logGPPdays10pCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp5CLIMdf <- sfandcatch(
  logGPPdaysCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp5_v10pdf <- sfandcatch(
  logGPPdaysvalleys10p5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp5_v10pCLIMdf <- sfandcatch(
  logGPPdaysvalleys10pCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp5_vCLIMdf <- sfandcatch(
  logGPPdaysvalleysCLIM5[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
# inputs
tempdf <- sfandcatch(temperature, polyinfo = ltimNoNorth, newname = 'Temp') %>%
  mutate(center = st_centroid(Shape), latpos = st_coordinates(center)[, 2])

climdf <- sfandcatch(climate, polyinfo = ltimNoNorth, newname = 'Temp') %>%
  mutate(center = st_centroid(Shape), latpos = st_coordinates(center)[, 2])
...
er10pCLIMdf <- sfandcatch(
  logERdays10pCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er_v10pdf <- sfandcatch(
  logERdaysvalleys10p[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er_v10pCLIMdf <- sfandcatch(
  logERdaysvalleys10pCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
er_vCLIMdf <- sfandcatch(
  logERdaysvalleysCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
gpp10pCLIMdf <- sfandcatch(
  logGPPdays10pCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppCLIMdf <- sfandcatch(
  logGPPdaysCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp_v10pdf <- sfandcatch(
  logGPPdaysvalleys10p[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp_v10pCLIMdf <- sfandcatch(
  logGPPdaysvalleys10pCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gpp_vCLIMdf <- sfandcatch(
  logGPPdaysvalleysCLIM[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
inunannualdf <- sfandcatch(
  inundationannual / 1000,
  polyinfo = ltimNoNorth,
  newname = 'inundation'
) %>%
  mutate(
...
inun10pannualdf <- sfandcatch(
  inundation10pannual / 1000,
  polyinfo = ltimNoNorth,
  newname = 'inundation'
) %>%
  mutate(
...
# outputs- don't do the uncertainty here yet.
erannualdf <- sfandcatch(
  logERdaysannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual10pdf <- sfandcatch(
  logERdays10pannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual10pCLIMdf <- sfandcatch(
  logERdays10pCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannualCLIMdf <- sfandcatch(
  logERdaysCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual_vdf <- sfandcatch(
  logERdaysvalleysannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual_v10pdf <- sfandcatch(
  logERdaysvalleys10pannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual_v10pCLIMdf <- sfandcatch(
  logERdaysvalleys10pCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
erannual_vCLIMdf <- sfandcatch(
  logERdaysvalleysCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'ER'
) %>%
  mutate(
...
gppannualdf <- sfandcatch(
  logGPPdaysannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual10pdf <- sfandcatch(
  logGPPdays10pannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual10pCLIMdf <- sfandcatch(
  logGPPdays10pCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannualCLIMdf <- sfandcatch(
  logGPPdaysCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual_vdf <- sfandcatch(
  logGPPdaysvalleysannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual_v10pdf <- sfandcatch(
  logGPPdaysvalleys10pannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual_v10pCLIMdf <- sfandcatch(
  logGPPdaysvalleys10pCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
...
gppannual_vCLIMdf <- sfandcatch(
  logGPPdaysvalleysCLIMannual[1, , ] / 1000,
  polyinfo = ltimNoNorth,
  newname = 'GPP'
) %>%
  mutate(
