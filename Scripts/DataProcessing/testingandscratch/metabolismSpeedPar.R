...
chunkpred <- function(bottom, top) {
  subCatch <- Murrumbidgee_weightedMean %>% slice('geometry', bottom:top) # %>% slice('time', 500:515)
  ...
  # predict new values
  catchSF <- catchSF %>%
    add_preds(gppDaysValleys, predname = 'logGPPdaysvalleys', interval = 'both', parTF = TRUE)  %>%
    add_preds(gppDays, predname = 'logGPPdays', interval = 'both', parTF = TRUE) %>%
    add_preds(erDaysValleys, predname = 'logERdaysvalleys', interval = 'both', parTF = TRUE) %>%
    add_preds(erDays, predname = 'logERdays', interval = 'both', parTF = TRUE)
