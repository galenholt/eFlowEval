# Script to build on the ANAE without having to reprocess it
library(here)
library(tidyverse)
library(sf)
library(stars)

# also loads internal functions
source('directorySet.R')

# load the processed anae files, cut to lachlan
load(file.path(datOut, 'LachlanANAE.rdata'))
# load(file.path(datOut, 'bothANAE.rdata'))

# To allow plotting the ltim zones (otherwise their polygons get lost)
# And get the LTIM_Valleys to use to subset for toy models at scale, but not enormous scale
LTIM_Valleys <- read_sf(dsn = file.path(datDir, 'ANAE/MDB_ANAE_Aug2017/MDB_ANAE.gdb'), layer = 'LTIM_Valleys') %>%
  st_cast("MULTIPOLYGON") # cleans up an issue with multisurfaces

# LTIM areas, useful for plotting
ltimNoNorth <- LTIM_Valleys %>%
  select(ValleyName, ValleyID, ValleyCode) # Three different ways to reference, basically


# Read in the soil data
soilMstars <- read_ncdf(file.path(datDir, 'soilmoisture/sm_pct_2010_Actual_day.nc'))

# Try for all
allmoistyears <- list.files(file.path(datDir, 'soilmoisture'), pattern = 'day.nc')
allpaths <- file.path(datDir, 'soilmoisture', allmoistyears)

soilMstars <- read_ncdf(allpaths[1])
soilMstars2 <- read_ncdf(allpaths[2])


# scratch to sort out RAM -------------------------------------------------

# So, that's going to fill up RAM. In theory, should be able to use start proxy objects. But unclear how that'll work with the rolling functions...
# AND, I think there was a problem even reading things in with read_stars instead of read_ncdf
# Going to just have to start sorting it out one thing at a time.
soilMstars <- read_ncdf(allpaths[1])
soilP <- read_stars(allpaths[1])
soilMstars
soilP

# That does something funny with attribute. Is it still there?
names(soilP)

# Can I ust change the name?
mean(soilP[,,,200], na.rm = TRUE)
mean(soilMstars[,,,200], na.rm = TRUE)

# No, soilP isn't actually finding the data
str(soilMstars$sm_pct)
str(soilP$sm_pct_2010_Actual_day.nc)
# Or, I'm misunderstanding how stars proxies work

# does read_stars work at all? ie is this a proxy issue or a read_stars issue?
soilPF <- read_stars(allpaths[1], proxy = FALSE)
soilPF

# do I get the data (ie do the evaluation that's waiting around) by
soilPs <- st_as_stars(soilP)
# OK, so that basically works. 
# Now, if i fix the name (or, can I avoid that?) and the coord system, can I do
# all the clipping and averaging into ANAE? and then will it fit to do the time
# roll? ie can I short-circuit the RAM problem for now?


# But, what happens with the names since it's stumbling on filenames?
soilP <- read_stars(allpaths[1:2])
soilPs <- st_as_stars(soilP)
# Ok, that's fine if it wants to just call it 'attr'. I can handle that.

# Let's see how much of this works
  # Get the crs from the original way of reading the netcdf
# whichcrs <- st_crs(soilMstarsOrig) # yields 4326
st_crs(soilP) <- 4326
st_crs(soilP)
# scs <- c(soilMstars, soilMstars2)

# Some of the plotting below seems off though
plot(soilMstars[,,,1:4])
plot(soilP[,,,1:4])
# Ok, lame subsampling, but whatever

# To check things, lets
smo <- soilMstars
# Let's start working through the below with the new version, and see how it goes
  # rename so i don't have to change following code
soilMstars <- soilP
rm(soilP)
rm(soilPs)

# Previous work -----------------------------------------------------------

# and the basin boundary, for clipping rasters; though likely will start with lachlan
basin <- read_sf(dsn = file.path(datDir, 'ANAE/MDB_ANAE_Aug2017/MDB_ANAE.gdb'), layer = 'MDB_Boundary') %>%
  st_cast("MULTIPOLYGON")  %>% # cleans up an issue with multisurfaces
  dplyr::select(LEVEL2NAME) # no need for other info

# Set the crs.
# Doing this here because it's not yet clear which is best, so we want
# everything in before we choose one to get the donor CRS from
# using a regular (not curvilinear) grid is necessary to avoid retaining the
# full extent and size of the raster
# soilMstars comes in as regular, so use that. Though st_warp() likely would
# do it on a different projection (see help and vignettes)
whichcrs <- st_crs(soilMstars)
# TODO:: Check this is right on PC, I didn't need to set the CRS there for some reason
st_crs(LachlanANAE) <- 4283 # This is what's there when I look at st_crs, but it's not registering for some reason. 
LachlanANAE <- st_transform(LachlanANAE, whichcrs)
ltimNoNorth <- st_transform(ltimNoNorth, whichcrs)
basin <- st_transform(basin, whichcrs)

# Crop. Have to use as_points = FALSE or it crops to the raster centers, and misses stuff around the edges.
lachSoil <- st_crop(soilMstars, filter(ltimNoNorth, ValleyName == 'Lachlan'), as_points = FALSE)
# With the non-proxy version for checking
lso <- st_crop(smo, filter(ltimNoNorth, ValleyName == 'Lachlan'), as_points = FALSE)

# check it worked
  # It does for the read_ncdf version, but unclear for the proxy, because of the funny order of operations
plot(lachSoil[,,,1:4], reset = FALSE)
plot(filter(ltimNoNorth, ValleyName == 'Lachlan'), add = TRUE, border = 'red')
plot(lso[,,,1:4]) # non-proxy

# Is it the order of operations when it pulls from the proxy object?
  # YES. Phew. so, the plot above was stuffing up the crop. This is going to be tricky to ensure we're doing things right.
proxTest <- st_as_stars(lachSoil)
plot(proxTest[,,,1:4])


# Let's set up a bbox for subsampled plotting without taking 8 million years
bb <- st_bbox(c(xmin = 144.4, ymin = -33.8, xmax = 144.6, ymax = -33.6), crs = whichcrs)

# And quick plots of the ANAE and rasters
anaeSub <- st_crop(LachlanANAE, st_as_sfc(bb))
plot(anaeSub[,'ANAE_DESC'])

# WHY is the bbox subbing backwards/inside out or something?
soilSub <- lachSoil[st_as_sfc(bb)]
soilunprox <- st_as_stars(soilSub)
sso <- lso[st_as_sfc(bb)]
plot(soilSub[,,,10:13])

# Look at the "To" column in the dimensions: what's happening here?
soilSub
sso
deproxbox <- proxTest[st_as_sfc(bb)]
deproxbox
plot(deproxbox[,,,10:13])
plot(sso[,,,10:13])

# So, the boxing is stuffing it up. It REALLY seems like the proxy struggles when there's a chain of things to do. So...
  # Could just stay with proxy until cut to lachlan. That's likely to be small
  # enough to then work with, at least for this data. It's kind of kicking the
  # can down the road, but not sure what else to do. Can always chunk up finer
  # data to get into the ANAE merge, and then that'll always be the same size...

  # Stay with proxy through the rastPolyJoin? Does that work? In this case, it will actually INCREASE the object size.
    # Does it remain a proxy?
    # Doesn't work

  # If that remains a proxy, can we do the timeroll?
      # no, because it fails in the first place. BUt no matter how we get to the
      # ANAE, we SHOULD be able to timeroll on it; ie if 5 years of ANAE in the
      # Lachlan fit in memory, they fit in memory. And if they don't, we can cut
      # to a smlaler spatial subset etc...


    # And even if not, can we timeroll on the raster as proxy?
      # A test from below
rastTimeSmean_7 <- lachSoil
rastTimeSmean_7[[1]] # This is NOT an array, but a list of names. IE the structure is different, so this kind of list fails.
  # basc=ically, the [[]] operator just sees a list; it's not doing anything special for stars. SO, is there a way to access the array?
  # AND, does it matter? The array will be nearly ALL the info, so once it's in
  # to do the roll, we've lost any advantage of the proxy anyway. If the roll
  # doesn't fit, we'll need to sort out an on-disk method for ROLLING, not for
  # starsing.
# system.time(rastTimeSmean_7[[1]] <- timeRoll(rastTimeSmean_7[[1]],
#                                              FUN = RcppRoll::roll_mean, 
#                                              rolln = 7, 
#                                              align = 'right',
#                                              na.rm = TRUE))

# 1. Spatial average of raster into the ANAE polys at each timestep ---------------------------

# Test the new function
  # Doesn't work wiht the stars_proxy lachSoil
system.time(dailySMpolyavg <- rastPolyJoin(polysf = LachlanANAE, rastst = lachSoil, grouper = 'SYS2', maintainPolys = TRUE))

# Runs, but need to check it actually worked and isn't stuffed by order of
# operations with proxTest,, whcih is st_as_stars(lachSoil)
system.time(dailySMpolyavg <- rastPolyJoin(polysf = LachlanANAE, rastst = proxTest, grouper = 'SYS2', maintainPolys = TRUE))

# 290 seconds with 280 timesteps in Lachlan
  # There has GOT to be a way to speed this up...
  # It's the grouped summarize that kills it, NOT the intersection. i think it
  # might be the work sf is doing to put the polygons back together?
    # TODO: see if it's faster to st_drop_geometry, do the summarize, and then join back to LachlanANAE with SYS2?

# unpack the list
dailyPolyindex <- dailySMpolyavg[[2]]
# at least for testing, don't overwrite, or have to run the whole thing again
dailyPolySMavg <- dailySMpolyavg[[1]]

# Check ordering
all(dailyPolyindex$SYS2 == LachlanANAE$SYS2)

# Test plot
dailyPolySub <- dailyPolySMavg[st_as_sfc(bb)]
plot(dailyPolySub[,,10:13])

# ggplot is a bigger pain, skip for now, we just want to look at things
# testDaily <- ggplot() + 
#   geom_stars(data = dailyPolySub[,,10:13], aes(fill = sm_pct)) +
#   facet_grid(cols = as.character)
# testDaily

# Rolling through time over the spatial polygons ---------------------

# Set up a new stars object from the daily soil moistures in the polygons
polyTimeSmax_10 <- dailyPolySMavg 

# Get a 10-day rolling max for each polygon
system.time(polyTimeSmax_10[[1]] <- timeRoll(polyTimeSmax_10[[1]], 
                                             FUN = RcppRoll::roll_max, 
                                             rolln = 10, 
                                             align = 'right',
                                             na.rm = TRUE))

# Half a second?? That's great!

# Test plot
max10PolySub <- polyTimeSmax_10[st_as_sfc(bb)]
plot(max10PolySub[,,10:13])

# Rolling through time on the raster, then putting into polygons --------

# Could probably streamline with the dailies in the first section, but for now
rastTimeSmean_7 <- lachSoil
system.time(rastTimeSmean_7[[1]] <- timeRoll(rastTimeSmean_7[[1]],
                                             FUN = RcppRoll::roll_mean, 
                                             rolln = 7, 
                                             align = 'right',
                                             na.rm = TRUE))
# 0.7 seconds
# Test plot the raster itself
rastweekSub <- rastTimeSmean_7[st_as_sfc(bb)]
plot(rastweekSub[,,,10:13])


# Now, put those time-means into polygons
system.time(weeklySMpoly <- rastPolyJoin(polysf = LachlanANAE, rastst = rastTimeSmean_7, grouper = 'SYS2', maintainPolys = TRUE))


# unpack the list
weeklyPolyindex <- weeklySMpoly[[2]]
# at least for testing, don't overwrite, or have to run the whole thing again
weeklyRastavgSMpoly <- weeklySMpoly[[1]]

# Check ordering
all(weeklyPolyindex$SYS2 == LachlanANAE$SYS2)

# Test plot
weekmeanSub <- weeklyRastavgSMpoly[st_as_sfc(bb)]
plot(weekmeanSub[,,10:13])



# Split the polygons by the rasters to get smaller scale ------------------

# Let's do this with the dailies, because then it would be easy to, for example, get a rolling min or something.

# Test the new function
system.time(dailySMpolysplit <- rastPolyJoin(polysf = LachlanANAE, 
                                             rastst = lachSoil, 
                                             grouper = 'SYS2', 
                                             maintainPolys = FALSE))
# 290 seconds with 280 timesteps in Lachlan
# There has GOT to be a way to speed this up...
# It's the grouped summarize that kills it, NOT the intersection. i think it
# might be the work sf is doing to put the polygons back together?
# TODO: see if it's faster to st_drop_geometry, do the summarize, and then join back to LachlanANAE with SYS2?

# unpack the list
dailyPolySplitindex <- dailySMpolysplit[[2]]
# at least for testing, don't overwrite, or have to run the whole thing again
dailyPolySMsplit <- dailySMpolysplit[[1]]

# Check ordering
  # This SHOULD be false
all(dailyPolySplitindex$SYS2 == LachlanANAE$SYS2)

# Test plot
dailyPolySplitSub <- dailyPolySMsplit[st_as_sfc(bb)]
plot(dailyPolySplitSub[,,10:13])


# all the test plots in one place -----------------------------------------
  #including making the subsets and bbox, so we can use something different here, or comment out above
# load(file.path(datOut, 'kanDemo.rdata'))

# it's a little annoying that st_crop is a hard crop, but the [] cropping lets
# the polys hang over, and so we get different extents. So doing the polygon
# crops a bit different than above, so we DO get the same extents

# Let's set up a bbox for subsampled plotting without taking 8 million years
bb = st_bbox(c(xmin = 144.4, ymin = -33.8, xmax = 144.6, ymax = -33.6), crs = whichcrs)

# ANAE
anaeSub <- st_crop(LachlanANAE, st_as_sfc(bb))
plot(anaeSub[,'ANAE_DESC'])

# Soil moisture raster
soilSub <- lachSoil[st_as_sfc(bb)]
plot(soilSub[,,,10:13])

# Daily sm in the polygons
dailyPolySub <- st_crop(st_as_sf(dailyPolySMavg[,,10:13]), st_as_sfc(bb))
plot(dailyPolySub)

# 10-day max in the polygons directly
max10PolySub <- st_crop(st_as_sf(polyTimeSmax_10[,,10:13]), st_as_sfc(bb))
plot(max10PolySub)

# Weekly mean in the raster (as a raster still)
rastweekSub <- rastTimeSmean_7[st_as_sfc(bb)]
plot(rastweekSub[,,,10:13])

# Weekly mean put into polygons
weekmeanSub <- st_crop(st_as_sf(weeklyRastavgSMpoly[,,10:13]), st_as_sfc(bb))
plot(weekmeanSub)

# and the split by raster units
dailyPolySplitSub <- st_crop(st_as_sf(dailyPolySMsplit[,,10:13]), st_as_sfc(bb))
plot(dailyPolySplitSub)

save(LachlanANAE, 
     lachSoil, 
     dailyPolySMavg, 
     polyTimeSmax_10, 
     weeklyRastavgSMpoly, 
     file = file.path(datOut, 'kanDemo.rdata'))



