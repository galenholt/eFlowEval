...
single_ramsar_poly <- filter(ramsarMDB, WNAME == params$ramsar) %>% 
  summarise()
...
scriptOut <- file.path('strictOut', 'veg', 'local', 
                       stringr::str_replace_all(params$ramsar, ' ', '_'))
...
```{r}
veg_local <- setup_local_veg(sub_poly = single_ramsar_poly, catchment = 'auto')
...
```{r}
centipeda_prop <- local_veg_plot(veg_local$centipedadf, datewanted, plotPkg = 'ggplot')
...
centipeda_prop_yr <- local_veg_plot(veg_local$centipedayrdf, 
                                 ...
                                 plotPkg = 'ggplot', titled = FALSE)


centipeda_prop_yr + pubtheme

ggsave(file.path(scriptOut, 'centipeda_yr.pdf'),
       plot = centipeda_prop_yr + pubtheme,
       width = 16, height = 16, units = 'cm')
ggsave(file.path(scriptOut, 'centipeda_yr.png'),
       plot = centipeda_prop_yr + pubtheme,
       width = 16, height = 16, units = 'cm')
```

save the one that looks interesting

```{r}
centipeda_prop_yr16 <- local_veg_plot(veg_local$centipedayrdf, 
...
centipeda_prop_yr16
# ggsave(file.path(scriptOut, 'centipeda_yr16.pdf'),
#        plot = centipeda_prop_yr16,
#        width = 16, height = 6, units = 'cm')
```

```{r}
ggsave(file.path(scriptOut, 'centipeda_yr16.png'),
       plot = centipeda_prop_yr16 + 
         scale_x_continuous(breaks = c(144.1, 144.5)) + 
         scale_y_continuous(breaks = c(-35.20, -35.40)),
       width = 16, height = 6, units = 'cm')
```
