...
I haven't moved the function to `\R` yet, so 
```{r}
source(file.path('Scripts', 'plotting', 'metabPlotSetup_Local.R'))
```

I pulled the saving to here, since the function doesn't auto-print. Do we want it to? Probably not, since we end up tweaking everything here.

```{r}
# where are we going to want to save?
scriptOut <- file.path('strictOut', 'metabolism', 'local', 'werai')
if (!dir.exists(scriptOut)) {dir.create(scriptOut, recursive = TRUE)}
```


New- use a function to make this general. That means for a particular situation, we need to set up where it runs and what it uses.
...
```{r}
metab_local <- setup_local_metab(sub_poly = werai_one, catchment = 'auto')

```

We could read in some other useful shapefiles here (basin and ltimNoNorth), but we might not actually use them here.
...
ltimNoNorth <- load_rename(catchpath, 'ltimNoNorth')[[1]]
```
...
# if (!('metab_local$inun_anae' %in% ls())) {
...
# allfun(tempObj = metab_local$temp_anae, tempAtt = 1,
#                    inunObj = metab_local$inun_anae, inunAtt = 1,
#                    gppObj = metab_local$predGPP, gppAtt = 1,
#                    erObj = metab_local$predER, erAtt = 1,
#                    datewanted)
...
```{r}
tm_grid_static <- tmap_arrange(tempfun(metab_local$temp_anae, 1, datewanted) +
                                 tm_layout(bg.color = "grey85"),
                               inunfun(metab_local$inun_anae, 1, datewanted, titled = FALSE) +
                                 tm_layout(bg.color = "grey85"),
                               gppfun(metab_local$predGPP, 1, datewanted, titled = FALSE) +
                                 tm_layout(bg.color = "grey85"),
                               erfun(metab_local$predER, 1, datewanted, titled = FALSE) +
                                 tm_layout(bg.color = "grey85"))
tm_grid_static
```
...
gg_grid_static <- ggpubr::ggarrange(
  tempfun(metab_local$temp_anae, 1, datewanted, titled = FALSE,
  ...
  inunfun(metab_local$inun_anae, 1, datewanted, titled = FALSE, plotPkg = 'ggplot') + 
    guides(fill = guide_legend(title.position = 'top')) +
    theme_pub() +
    theme(legend.position = 'bottom', 
          legend.background = element_blank(),
          legend.key.size = unit(0.3, 'cm')),
  gppfun(metab_local$predGPP, 1, datewanted, titled = FALSE, plotPkg = 'ggplot') + 
    guides(fill = guide_legend(title.position = 'top')) +
    theme_pub() +
    theme(legend.position = 'bottom', 
          legend.background = element_blank(),
          legend.key.size = unit(0.3, 'cm')),
  erfun(metab_local$predER, 1, datewanted, titled = FALSE, plotPkg = 'ggplot') + 
    guides(fill = guide_legend(title.position = 'top')) +
    theme_pub() +
    theme(legend.position = 'bottom', 
          legend.background = element_blank(),
...
erControl <- ersetup(data = metab_local$predER, attnum = 1)
...
gpp_sf <- metab_local$predGPP[1,,] %>% 
  st_as_sf() %>% 
  select(all_of(datewanted)) %>%
  rename(GPP = 1) %>%
  mutate(logGPP = log10(1+GPP), type = 'estimate')
gpp_sfPU <- metab_local$predGPP[2,,] %>% 
  st_as_sf() %>% 
  select(all_of(datewanted)) %>%
  rename(GPP = 1) %>%
  mutate(logGPP = log10(1+GPP), type = 'upper')
gpp_sfPL <- metab_local$predGPP[3,,] %>% 
  st_as_sf() %>% 
  select(all_of(datewanted)) %>%
  rename(GPP = 1) %>%
...
gppControl <- gppsetup(metab_local$predGPP, attnum  = 1)
...
inun_sf <- metab_local$inun_anae[1,,] %>% 
  st_as_sf() %>% 
  select(all_of(datewanted)) %>%
  rename(inun = 1) %>%
...
xbreaks <- foreach(b = breakswanted, .combine = c) %dofuture% {
...
# GPP
gppYear_sf <- metab_local$gppannual %>% 
  st_as_sf() %>% 
...
# ER
erYear_sf <- metab_local$erannual %>% 
  st_as_sf() %>% 
...
# Temp
tempYear_sf <- metab_local$tempannual %>% 
  st_as_sf() %>% 
  pivot_longer(cols = -geometry, names_to = 'WaterYear', values_to = names(metab_local$tempannual)) %>%
  st_as_sf() # REALLY???


# Inundation
inunYear_sf <- metab_local$inunannual %>% 
  st_as_sf() %>% 
  pivot_longer(cols = -geometry, names_to = 'WaterYear', values_to = names(metab_local$inunannual)) %>%
...
# aggregate to werai
# temp area-weighted
areas <- metab_local$tempannual %>%
  st_geometry() %>%
  st_area() %>%
  as.numeric()

tempW <- catchAggW(strict = metab_local$tempannual, strictWeights = areas,
                               FUN = mean, summaryPoly = werai_one)
  # sum across space
inunW <- aggregate(metab_local$inunannual, 
                      by = werai_one, 
                      FUN = sum, na.rm = TRUE)

gppW <- aggregate(metab_local$gppannual, 
                   by = werai_one, 
                   FUN = sum, na.rm = TRUE)
erW <- aggregate(metab_local$erannual, 
                   by = werai_one, 
                   FUN = sum, na.rm = TRUE)
...
gppyrlabel <- 'Yearly GPP maximum\n(kg 02)'
  ...
  metab_local$gppannual_tm <- gppYear_sf %>%
...
eryrlabel <- 'Yearly ER maximum\n(kg 02)'
  ...
  metab_local$erannual_tm <- erYear_sf %>%
    filter(WaterYear != '2014-06-29') %>% # because a 5-panel is ugly
    tm_shape() +
    tm_fill(col = 'logER',
...
```{r}
metab_local$gppannual_gg <- gppYear_sf %>%
    filter(WaterYear != '2014-06-29') %>% # because a 5-panel is ugly
    ggplot() +
  ...
  metab_local$gppannual_gg
```

#### ER

```{r}
metab_local$erannual_gg <- erYear_sf %>%
    filter(WaterYear != '2014-06-29') %>% # because a 5-panel is ugly
    ggplot() +
...
tempControl <- tempsetup(data = metab_local$temp_anae, attnum = 1)
metab_local$tempannual_gg <- tempYear_sf %>%
    filter(WaterYear != '2014-06-29') %>% # because a 5-panel is ugly
    ggplot() +
...
inunControl <- inunsetup(data = metab_local$inun_anae, attnum = 1)
metab_local$inunannual_gg <- inunYear_sf %>%
    filter(WaterYear != '2014-06-29') %>% # because a 5-panel is ugly
    ggplot() +
    ...
    scale_fill_stepsn(colors = inunControl$inunpal,
                      ...
                      name = inunyrlabel) +
    facet_wrap(vars(WaterYear))
 metab_local$inunannual_gg
```

Stack 3 years of gpp and ER together for the document

```{r}
metab_local$gppannual_gg3 <- gppYear_sf %>%
  ...
  # metab_local$gppannual_gg3 
  
  metab_local$erannual_gg3 <- erYear_sf %>%
    filter(WaterYear != '2014-06-29' & WaterYear != '2018-06-29') %>% # because a 5-panel is ugly
      mutate(WaterYear = str_remove(WaterYear, '-06-29')) %>% # because these are water years, don't clutter up wit months
    ggplot() +
    geom_sf(mapping = aes(fill = logER), color = NA) +
    ...
    facet_grid(WaterYear~.) + 
    theme_pub(legend.position = 'bottom')
  # metab_local$erannual_gg3

  annualGPPER <- ggpubr::ggarrange(metab_local$gppannual_gg3 + 
                                     guides(fill = guide_legend(title.position = 'top')) +
                                     theme_pub() +
                                     theme(legend.position = 'bottom', 
                                           legend.background = element_blank(),
...
```{r}
metab_local$tempannual_gg3 <- tempYear_sf %>%
    filter(WaterYear != '2014-06-29' & WaterYear != '2018-06-29') %>% # because a 5-panel is ugly
  mutate(WaterYear = str_remove(WaterYear, '-06-29')) %>% # because these are water years, don't clutter up wit months
    ggplot() +
    geom_sf(mapping = aes(fill = bimonth_tempC), color = NA) +
  ...
  metab_local$inunannual_gg3 <- inunYear_sf %>%
    filter(WaterYear != '2014-06-29' & WaterYear != '2018-06-29') %>% # because a 5-panel is ugly
  mutate(WaterYear = str_remove(WaterYear, '-06-29')) %>% # because these are water years, don't clutter up wit months
    ggplot() +
  ...
  annualTI <- ggpubr::ggarrange(metab_local$tempannual_gg3 + 
                                     guides(fill = guide_legend(title.position = 'top')) +
                                     theme_pub() +
                                     theme(legend.position = 'bottom', 
                                           legend.background = element_blank(),
                                           legend.key.size = unit(0.3, 'cm')), 
                    metab_local$inunannual_gg3 + 
                      guides(fill = guide_legend(title.position = 'top')) +
                      theme_pub() +
                      theme(legend.position = 'bottom', 
                            legend.background = element_blank(),
...
Well, wait. I already have metab_local$erannual and metab_local$gppannual and metab_local$inunannual and
metab_local$tempannual here as stars. so I think I don't need to go wholly remake
...
# Sort out the scales
yrdex <- which(st_get_dimension_values(metab_local$tempannual, which = 'time') == yrwanted)

metab_local$tempannual[,,yrdex]
metab_local$climannual[,,yrdex]

metab_local$inunannual[,,yrdex]
metab_local$inun10pannual[,,yrdex]

# Need to capture the endmembers
metab_local$gppannual[1,,yrdex]
metab_local$gppannual_10p_clim[,,yrdex]

metab_local$erannual[1,,yrdex]
metab_local$erannual_10p_clim[,,yrdex]
...
gppBase <- gppfun(metab_local$gppannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
gppClim <- gppfun(metab_local$gppannual_clim, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
gpp10p <- gppfun(metab_local$gppannual_10p, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
gpp10p_clim <- gppfun(metab_local$gppannual_10p_clim, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
```{r}
erBase <- erfun(metab_local$erannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
erClim <- erfun(metab_local$erannual_clim, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
er10p <- erfun(metab_local$erannual_10p, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
er10p_clim <- erfun(metab_local$erannual_10p_clim, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
tempyr <- tempfun(metab_local$tempannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
climyr <- tempfun(metab_local$climannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
inunyr <- inunfun(metab_local$inunannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
inun10pyr <- inunfun(metab_local$inun10pannual, 1, 
                 datewanted = yrwanted,
                 # units = 'tonnes', 
                 plotPkg = 'ggplot', 
                 titled = FALSE,  
...
```{r}
ggplot() +
  geom_stars(data = metab_local$gppannual[1,,1], mapping = aes(fill = predicted_GPP_kg02perday_at_max_inun)) +
  geom_stars(data = metab_local$gppannual[1,600:700,1], fill = 'white')
```
...
gppannclip <- metab_local$gppannual[1, clipset, ]
gppannclip_clim <- metab_local$gppannual_clim[1, clipset, ]

gppannclip10p <- metab_local$gppannual_10p[1, clipset, ]
gppannclip10p_clim <- metab_local$gppannual_10p_clim[1, clipset, ]
...
```{r}
tempclip <- metab_local$tempannual[1,clipset, ]
climclip <- metab_local$climannual[1, clipset, ]

inunclip <- metab_local$inunannual[1,clipset, ]
refinun <- inunclip
inunclip <- inunclip - refinun

inun10pclip <- metab_local$inun10pannual[1,clipset,]
