...
```{r}
check_surv <- test_anae_agg(catchments = NULL, datDir = datOut, variableDir = 'Inundationprocessed', summaryFun = 'lippiaAdultSurvive', forceAllCatchments = TRUE)
...
```{r}
check_temp <- test_anae_agg(catchments = NULL, datDir = datOut, variableDir = 'Tempprocessed', summaryFun = 'weightedMean', forceAllCatchments = TRUE)
...
surv_fail <- check_surv %>% 
  filter(passfail != 'pass' & !grepl('duplicates are failing to match, but seem to actually match', passfail))
